							...
							<table class='table table-striped'>
								...
								<tr>
									...
									<td>
										<div class="form-group">
											<select class="select2 form-control" name="produk_detail[]" multiple="multiple" id="multiple-basic">
												<?php
												$this->db->where('aktif', 'Y');
												$result = $this->db->get('m_produk_detail')->result();
												foreach ($result as $dt) {
													echo '<option value="' . $dt->id_produk_detail . '">' . $dt->nm_produk_detail . '</option>';
												}
												?>
											</select>
										</div>
								...
								<tr>
									...
									<td>
										...
										<input type="hidden" name="id_pesanan" value="<?php echo $maxx; ?>" />
...
<script>
	$(document).ready(function() {
		...
		$('#produk').change(function() {
			...
			if (id_produk != '') {
				$.ajax({
					url: "<?php echo base_url(); ?>t_pesanan/fetch_produk_sub",
					method: "POST",
					data: {
						id_produk: id_produk
					},
		...
		$('#produk_sub').change(function() {
			...
			if (id_produk_sub != '' && id_produk != '') {
				$.ajax({
					url: "<?php echo base_url(); ?>t_pesanan/fetch_produk_harga",
					method: "POST",
					data: {
						id_produk: id_produk,
